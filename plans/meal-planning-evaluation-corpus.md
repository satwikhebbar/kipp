...
| 6 | holiday-half-day | Saturday `school_closed`: coverage excludes Sat, and a Sat cell fails with `extra_slot_for_closed_day`. Wednesday `half_day` requires breakfast, snack 1, and home lunch only; snack 2 and packed school lunch are forbidden. A dry catalog item flagged `halfDaySnack` may cook for up to 20 minutes in snack 1 on that day only. | extra_slot_for_closed_day, missing_slot, slot_unsuitable |
