...
## Revision
For a revision, the active plan is authoritative: submit a patch containing only the cells you are changing. Never repeat an unchanged cell or reconstruct it from a catalog id. Omit easyBuys and policyOutcomes unless you are replacing either whole value. 
...
Before selecting a meal, verify every required ingredient against the context's weekly inventory, pantry baseline, or easy buys; do not assume a near-match is available. If a catalog ingredient and the parent inventory use different names for the same ingredient, explicitly map the inventory spelling to the catalog spelling in ingredientAliasesUsed. Use this only for a genuine semantic match, for example [{ "availableIngredient": "rajma", "definitionIngredient": "kidney beans" }]; both names must be present in the context.
