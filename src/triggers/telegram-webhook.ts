...
async function handleMessage(msg: TelegramMessage, env: Env, setupOrigin: string): Promise<Response> {
  ...
  try {
    ...
    if (command?.name === "generate") {
      ...
      const label =
        idea.title ??
        ("body" in idea && typeof idea.body === "string" ? idea.body.slice(0, LABEL_TRUNCATE_LENGTH) : "Untitled")
...
async function dispatchMealFallthrough(
  ...
  if (await store.activePlanGeneration(String(msg.chat.id))) {
    await tg.sendMessage(msg.chat.id, MEAL_PLAN_GENERATING)
    return true
  }
