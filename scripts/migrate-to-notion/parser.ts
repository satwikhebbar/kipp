...
export interface LegacyCorrelation {
...
/** The GitHub frontmatter idea shape that the Notion migration reads from ideas.md and archive.md. */
export interface LegacyIdea {
  id: string
  title?: string
  status: LegacyIdeaStatus
  created: string
  source: LegacySource
  body: string
  draft?: string
...
/** Converts one frontmatter-delimited Markdown idea into the legacy normalized Idea model. */
export function parseIdea(text: string): LegacyIdea {
  ...
  const idea: LegacyIdea = {
    id: String(fm.id ?? ""),
    title: fm.title ? String(fm.title) : undefined,
    status: (fm.status as LegacyIdeaStatus) ?? "raw",
    created: String(fm.created ?? ""),
    source: (fm.source as LegacySource) ?? "manual",
    body: preamble,
    draft,
    critique,
  }
...
function buildBody(idea: LegacyIdea): string {
...
export function serializeIdea(idea: LegacyIdea): string {
  const fm: FM = {
    id: idea.id,
    status: idea.status,
...
export function parseIdeas(text: string): LegacyIdea[] {
  const ideas: LegacyIdea[] = []
